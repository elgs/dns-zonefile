...
';
    ...
    var generate = function (options, template) {
        ...
        template = processOrigin(options['$origin'], template);
        template = processTTL(options['$ttl'], template);
        ...
        template = processURI(options['uri'], template);
        template = processValues(options, template);
        return template.replace(/\n{2,}/gim, '\n\n');
    };

    var processOrigin = function (data, template) {
        var ret = '';
    ...
    var processTTL = function (data, template) {
        var ret = '';
        if (typeof data !== 'undefined') {
            ret += '$TTL ' + data;
        }
    ...
    var processURI = function (data, template) {
        var ret = '';
        for (var i in data) {
           ret += (data[i].name || '@') + '\t';
           if (data[i].ttl) ret += data[i].ttl + '\t';
