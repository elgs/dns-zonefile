...
//                                                                                 //
/////////////////////////////////////////////////////////////////////////////////////
const generate = (options, template) => {
...
const process$ORIGIN = (data, template) => {
    let ret = ``;
...
const process$TTL = (data, template) => {
    let ret = ``;
    if (typeof data !== `undefined`) {
        ret += `$TTL ` + data;
    }
...
const processSOA = (data, template) => {
    let ret = template;
    data.name = data.name || `@`;
    data.ttl = data.ttl || ``;
...
const processAAAA = (data, template) => {
    let ret = ``;
    for (const value of data) {
        ret += (value.name || `@`) + `\t`;
        if (value.ttl)
...
const processCNAME = (data, template) => {
    let ret = ``;
    for (const value of data) {
        ret += (value.name || `@`) + `\t`;
        if (value.ttl)
...
const processPTR = (data, template) => {
    let ret = ``;
    for (const value of data) {
        ret += (value.name || `@`) + `\t`;
        if (value.ttl)
...
const processTXT = (data, template) => {
    let ret = ``;
    for (const value of data) {
        ret += (value.name || `@`) + `\t`;
        if (value.ttl)
...
const processSRV = (data, template) => {
    let ret = ``;
    for (const value of data) {
        ret += (value.name || `@`) + `\t`;
        if (value.ttl)
...
const processSPF = (data, template) => {
    let ret = ``;
    for (const value of data) {
        ret += (value.name || `@`) + `\t`;
        if (value.ttl)
...
const processCAA = (data, template) => {
    let ret = ``;
    for (const value of data) {
        ret += (value.name || `@`) + `\t`;
        if (value.ttl)
...
const processTLSA = (data, template) => {
    let ret = ``;
    for (const value of data) {
        ret += (value.name || `@`) + `\t`;
        if (value.ttl)
...
const processValues = (options, template) => {
...
const parseNS = (rrData, recordsSoFar) => {
...
const parseA = (rrData, recordsSoFar) => {
    const rrTokens = rrData.tokens;
    if (!rrData.hasName) {
        if (recordsSoFar.length) {
            rrTokens.unshift(recordsSoFar[recordsSoFar.length - 1].name);
...
const parseAAAA = (rrData, recordsSoFar) => {
    const rrTokens = rrData.tokens;
    if (!rrData.hasName) {
        if (recordsSoFar.length) {
            rrTokens.unshift(recordsSoFar[recordsSoFar.length - 1].name);
...
const parseCNAME = (rrData, recordsSoFar) => {
    const rrTokens = rrData.tokens;
    if (!rrData.hasName) {
        if (recordsSoFar.length) {
            rrTokens.unshift(recordsSoFar[recordsSoFar.length - 1].name);
...
const parseMX = (rrData, recordsSoFar) => {
    const rrTokens = rrData.tokens;
    if (!rrData.hasName) {
        if (recordsSoFar.length) {
            rrTokens.unshift(recordsSoFar[recordsSoFar.length - 1].name);
...
const parseTXT = (rrData, recordsSoFar) => {
    const rrTokens = rrData.tokens;
...
const parsePTR = (rrData, recordsSoFar, currentOrigin) => {
    const rrTokens = rrData.tokens;
...
const parseSRV = (rrData, recordsSoFar) => {
    const rrTokens = rrData.tokens;
    if (!rrData.hasName) {
        if (recordsSoFar.length) {
            rrTokens.unshift(recordsSoFar[recordsSoFar.length - 1].name);
...
const parseSPF = (rrData, recordsSoFar) => {
    const rrTokens = rrData.tokens;
    const txtArray = rrTokens.slice(rrData.typeIndex + 1);
    if (!rrData.hasName) {
        if (recordsSoFar.length) {
...
const parseCAA = (rrData, recordsSoFar) => {
    const rrTokens = rrData.tokens;
    if (!rrData.hasName) {
        if (recordsSoFar.length) {
            rrTokens.unshift(recordsSoFar[recordsSoFar.length - 1].name);
...
const parseDS = (rrData, recordsSoFar) => {
    const rrTokens = rrData.tokens;
    if (!rrData.hasName) {
        if (recordsSoFar.length) {
            rrTokens.unshift(recordsSoFar[recordsSoFar.length - 1].name);
...
const parseTLSA = (rrData, recordsSoFar) => {
    const rrTokens = rrData.tokens;
    if (!rrData.hasName) {
        if (recordsSoFar.length) {
            rrTokens.unshift(recordsSoFar[recordsSoFar.length - 1].name);
    ...
    if (rrData.hasTtl)
        result.ttl = parseInt(rrTokens[1]);
    return result;
};
