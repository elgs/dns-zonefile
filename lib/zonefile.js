...
';
    ...
    var parseRRs = function (text) {
        ...
        for (var i in rrs) {
            ...
            var rrArray = splitArgs(rr);
            var rrArrayWithQuotes = splitArgs(rr, null, true);
            if (rrArrayWithQuotes.indexOf('TXT') >= 0) {
                ret.txt = ret.txt || [];
                ret.txt.push(parseTXT(rrArray));
            } else if (rrArrayWithQuotes.indexOf('$ORIGIN') === 0) {
                ret.$origin = rrArray[1];
            } else if (rrArrayWithQuotes.indexOf('$TTL') === 0) {
                ret.$ttl = rrArray[1];
            } else if (rrArrayWithQuotes.indexOf('SOA') >= 0) {
                ret.soa = parseSOA(rrArray);
            } else if (rrArrayWithQuotes.indexOf('NS') >= 0) {
                ret.ns = ret.ns || [];
                ret.ns.push(parseNS(rrArray));
            } else if (rrArrayWithQuotes.indexOf('A') >= 0) {
                ret.a = ret.a || [];
                ret.a.push(parseA(rrArray, ret.a));
            } else if (rrArrayWithQuotes.indexOf('AAAA') >= 0) {
                ret.aaaa = ret.aaaa || [];
                ret.aaaa.push(parseAAAA(rrArray));
            } else if (rrArrayWithQuotes.indexOf('CNAME') >= 0) {
                ret.cname = ret.cname || [];
                ret.cname.push(parseCNAME(rrArray));
            } else if (rrArrayWithQuotes.indexOf('MX') >= 0) {
                ret.mx = ret.mx || [];
                ret.mx.push(parseMX(rrArray));
            } else if (rrArrayWithQuotes.indexOf('PTR') >= 0) {
                ret.ptr = ret.ptr || [];
                ret.ptr.push(parsePTR(rrArray, ret.ptr, ret.$origin));
            } else if (rrArrayWithQuotes.indexOf('SRV') >= 0) {
                ret.srv = ret.srv || [];
                ret.srv.push(parseSRV(rrArray));
            } else if (rrArrayWithQuotes.indexOf('SPF') >= 0) {
                ret.spf = ret.spf || [];
                ret.spf.push(parseSPF(rrArray));
            }
        }
        return ret;
    };
    ...
    var parseAAAA = function (rrTokens) {
        var l = rrTokens.length;
        var result = {
            name: rrTokens[0],
            ip: rrTokens[l - 1]
    ...
    var parseCNAME = function (rrTokens) {
        var l = rrTokens.length;
        var result = {
            name: rrTokens[0],
            alias: rrTokens[l - 1]
    ...
    var parseTXT = function (rrTokens) {
        var l = rrTokens.length;
        var result = {
            name: rrTokens[0],
            txt: rrTokens[l - 1]
    ...
    var parseSRV = function (rrTokens) {
        var l = rrTokens.length;
        var result = {
            name: rrTokens[0],
            target: rrTokens[l - 1],
    ...
    var parseSPF = function (rrTokens) {
        var result = {
            name: rrTokens[0],
            data: ''
        };
        ...
        var l = rrTokens.length;
        result.data = rrTokens[l - 1];

        if (!isNaN(rrTokens[1])) result.ttl = parseInt(rrTokens[1], 10);
        return result;
    };

    var splitArgs = function (input, separator, keepQuotes) {
