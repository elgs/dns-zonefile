...
describe("forward parser", () => {
  ...
  it("should parse txt records", () => {
    ...
    expect(records?.[0]?.["txt"]).toEqual(
      '"v=spf1 a mx a:mail.treefrog.ca a:webmail.treefrog.ca ip4:76.75.250.33 ?all" "asdfsdaf" "sdfsadfdasf"'
    );
    expect(records?.[1]?.["name"]).toEqual("treefrog.ca.");
    expect(records?.[1]?.["txt"]).toEqual(
      '"v=spf1 a mx a:mail.treefrog.ca a:webmail.treefrog.ca ip4:76.75.250.33 ?all" "asdfsdaf" sdfsadfdasf'
    );
  ...
  it("should parse spf records", () => {
    ...
    expect(records?.[2]?.["data"]).toEqual(
      '"v=spf3" "mx:gcloud-node.com." "-all    " "aasdfsadfdsafdasf"'
    );
